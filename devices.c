...
int main() {
    ...
    for (i = 0; i < platformCount; i++) {
        ...
        // get all devices
        int status = clGetDeviceIDs(platforms[i], CL_DEVICE_TYPE_ALL, 0, NULL, &deviceCount);
        if (status != CL_SUCCESS) {
            printf("Unable to get device IDs for platform %p, code %d\n", platforms[i], status);
            continue;
        }
        devices = (cl_device_id*) malloc(sizeof(cl_device_id) * deviceCount);
        status = clGetDeviceIDs(platforms[i], CL_DEVICE_TYPE_ALL, deviceCount, devices, NULL);
       	if (status != CL_SUCCESS) {
            printf("Unable to get device IDs detail for platform %p, code %d\n", platforms[i], status);
            continue;
        }
